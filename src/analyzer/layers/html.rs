...
impl TechnologyAnalyzer {
    ...
    pub(crate) fn scan_html_generic(&self, html: &str, detected: &mut HashMap<String, TechDetection>) {
        ...
        if let Some(gen) = generator {
            ...
            // Strip boilerplate the generator tag wraps around the product name.
            // WordPress plugins commonly write "Powered by <Product> <version> - <blurb>",
            // and matching the whole string against the database fails, so the fallback
            // below used to invent a technology literally named
            // "Powered by Slider Revolution" alongside the real "Slider Revolution"
            // detected from its assets — one product reported twice, under two names.
            ...
            let mut core_name = gen_name.as_str();
            loop {
                let lower = core_name.to_lowercase();
                match BOILERPLATE_PREFIXES.iter().find(|p| lower.starts_with(**p)) {
                    Some(prefix) => core_name = core_name[prefix.len()..].trim_start(),
                    None => break,
                }
            }

            // Try progressively shorter candidates against the DB, dropping one trailing
            // word at a time. "All in One SEO (AIOSEO)" only matches once "(AIOSEO)" is
            // dropped; previously the whole string and the first word were the only
            // candidates, so it became its own pseudo-technology.
            let mut candidates: Vec<String> = Vec::new();
            for name in [core_name, gen_name.as_str()] {
            ...
            // Surface even unknown generators so callers can see something, but under the
            // stripped name — "LayerSlider", not "Powered by LayerSlider".
            if !found && !core_name.is_empty() && core_name.len() < 64 {
                Self::update_detection(detected, core_name, "html", "meta-generator", 75, gen_version);
            }
        }
